...
userSchema.methods.createPasswordResetToken = function () {
  // The password reset token should basically be a random string but at the same time, it does not need to be a cryptographically strong
  // as the password hash that we created b4. So we can just use the very simple , random bytes fc from the built-in cryto module. So
  // lets import the "crypto" module above. Its a built-in module, so no need to install anything
  ...
  // We will creat a new field in our database schema where we will store this token, bcos of course, we want to save it in the database,
  // so we can then compare it with the token that the user provides. So we will add "passwordResetToken" and "passWordResetExpires" to our
  // schema
