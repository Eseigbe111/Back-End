...
// REFACTORING RESPONSE code for logging in user
const createSendToken = (user, statusCode, res) => {
  // To use JWT, we import it also on the top of our file:
  const token = signToken(user._id);


  // Sending JWT via Cookie: This part shows how to send tokens thru cookies
  // A COOKIE is a small piece of text a server sends to the client.
  // Browser stores it and sends it automatically for future requests.
  // 1)  Creating a Cookie setting
  const cookieOptions = {
    //a) The cookie will expire after a certain number of days (configured in .env)
    expires: new Date(
      Date.now() + process.env.JWT_COOKIE_EXPIRES_IN * 24 * 60 * 60 * 1000,
    ), // Converting days to milliseconds

    // b) secure: true, // If enabled , cookie is sent only over HTTPS (recommended for production).

    // c)This ensures that Cookie cannot be accessed by JavaScript on the browser → protects from XSS attacks.
    httpOnly: true,
  };

  // 2) Enabling secure cookies only in production: Add secure: true only in production (where HTTPS exists).
  if (process.env.NODE_ENV === 'production') cookieOptions.secure = true;

  // 3) Actually send the cookie: Create a cookie named jwt and send the token inside it.
  res.cookie('jwt', token, cookieOptions);

  // 4) Hide password before sending user data back : Remove the password from the response output.
