...
const userSchema = new mongoose.Schema({
  ...
  active: {
    type: Boolean,
    default: true, // Every new user created will have: meaning The user is active and Their account is not deleted
    select: false, // he active field will NOT be returned in queries by default. For e.g When you do: User.find(),
    // The returned objects will NOT show: "active": true or false. This is bcos
  },
});
...
//////////////////
// Pre(/^find/) middlewae for deleting Current User
// The reason for the below pre(/^find/) middleware is that:
// We don’t want inactive users to show up when fetching all users.
userSchema.pre(/^find/, function (next) {
  //^find  is a regular expression that means we are looking for words or strings that start with "find".This "/" ends the regular expression

  // SInce this is a query middleware, "this" pts to the current query
