...
module.exports = (err, req, res, next) => {
  ...
  if (process.env.NODE_ENV === 'development') {
    ...
    //  HAndling errors that maybe cause by jwt(token)
    //1) Error as a result of manipulated token  "jsonwebTokenError"
    if (error.name === 'jsonwebTokenError') error = handleJWTError(error);

    //2) Error from Expired token "TokenExpiredError"
