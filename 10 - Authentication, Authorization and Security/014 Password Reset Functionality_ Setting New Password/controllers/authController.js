// Importing the built-in crypto package i.e no need to install anything
...
exports.resetPassword = catchAsync(async (req, res, next) => {
  ...
  // So remember from our prev video, that the reset token that is actually sent in the URL is the non-encrypted token. But the one in the database
  // is the encrypted one. And so what we now need to do, is to basically encrypt the original token again so we can then compare it with the one
  // that is stored, so the encrypted one in the database.
  ...
  //d) Saving the doc
  // In this case, we don't need to turn off the validators, bcos indeed we want to validate e.g we want the validator to confirm if the password is equal to passwordConfirm
  await user.save();
