...
userSchema.pre('save', function (next) {
  ...
  next();
  // In theory the below code should work just fine, but smts a small prob happens. And that prob is that smt saving to the database
  // i a bit slower than issuing the JWT, making it so that the changed password timestamp s smts set a bit after the JWT has been
  // created.And so that will then make it so that the user will not ba able to log in using the new token. Bcos remember, the whole
  // reason this timestamps i.e passwordChangedAt  actually exists, is so that we can compare it with the timestamps in the JWT. So
  // to take care of that we just need to subtract 1sec i.e 1000millisec from "Date.now()" as seen above "Date.now() - 1000". So this
  // will always ensure that the token is created after the password has been changed.
});
// Ends here
