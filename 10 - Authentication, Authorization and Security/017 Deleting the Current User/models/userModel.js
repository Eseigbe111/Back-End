...
const userSchema = new mongoose.Schema({
  ...
  active: {
    type: Boolean,
    default: true,
    select: false, // We do not want to show this in the output as we do not want any one
    // to know that this active flag is here
  },
  //Ends here
});
...
// THIS IS FOR THIS LECTURE
//  Making the user not visible when we get All users but still in the database.
userSchema.pre(/^find/, function (next) {
  ...
  this.find({ active: { $ne: false } }); // here we want to find docs with active: true
  next();
});
