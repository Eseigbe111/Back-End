...
exports.forgotPassword = catchAsync(async (req, res, next) => {
  ...
  /// THIS IS FOR THIS LECTURE
  // SO we need to now send the password reset token via email to the user. And so in this lecture, we're gonna learn how to send
  // email using a very popular soln called "Nodemailer".
  // Let's now create an email handler fc that we can use thru our applicatn. And we will create it in our utils "folder" called
  // "email.js".We move to email.js

  //After creating the "sendEmail()" in "email.js", we can then continue with the code below:
