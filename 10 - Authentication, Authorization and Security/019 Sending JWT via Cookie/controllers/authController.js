...
const createSendToken = (user, statusCode, res) => {
  ...
  /// THIS IS FOR THIS LECTURE: Sending JWT via Cookie
  // So we learned in the last lecture that the JWT should be stored in a secure HTTP-only cookie. But right now, we're only sending the token
  // as a simple string in our JSON response.
  // In this video, let's also send the token as a cookie, so that the browser can then save it in this more secure way.

  // Let's briefly talk about cookie:
  // A COOKIE is basically just a small piece of text that a server can send to clients. Then when the client receives  a cookie, it will automatically
  // store it and then automatically send it back along with all future requests to the same server. For now this is not giong to to be useful to
  // us since we are dealing with postman, but whe we are rendering the dynamic pages later, and then really interact with the browser, then it
  // will bcom really important that the browser sends back the token basically automatically in each request.

  // So now let's lean how to create and send a cookie:
  // To send a cookie, we just basically attach it to the response object as seen below:

  const cookieOptions = {
    // The 1st parameter is the name of the cookie, 2nd is is the data we actually want to send in the cookie, and in our case it's the token variable
    // and 3rd is a couple of optns for the cookie
    expires: new Date(
      Date.now() + process.env.JWT_COOKIE_EXPIRES_IN * 24 * 60 * 60 * 1000,
    ), //Converting to millisecs
    // This "expires" ppt will make it so that the browser or the client in general will delete the cookie after it has expired.

    //secure makes the cookie to be sent on an encrypted connectn i.e only when using https
    // secure: true,

    // httpOnly makes it so that the cookie can not be accessed or modifed in any way by the browser. This is important in order t prevent those cross-site
    // scripting attacks.
    httpOnly: true,
  };
  // Now if we wanted to test this right now, it wouldn't work bcos right now, we're actually not using HTTPS. And so bcos of the "secure: true", the
  // cookie would not be created and not be sent to the client. And so we just only want to activate this part "secure: true" in prodn. So what i am
  // gonna do is to basically export the object of optns into a separate variable called "cookieOptions" and do the below

  if (process.env.NODE_ENV === 'production') cookieOptions.secure = true; // So with this we can comment secure: true, from the object
  ///
  res.cookie('jwt', token, cookieOptions);

  //Removing the password from the output when we want to create a cookie
  user.password = undefined;

  // To test this, watch how jonas did it .
