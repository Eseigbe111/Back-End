...
// //////////
// THIS IS FOR THIS LECTURE
// PREVENTING PARAMETER POLLUTION: we do "npm i hpp"
// Parameter pollution happens when an attacker (or faulty request) sends duplicate query parameters, which can break your logic or cause
// unexpected behavior.
// 1) Why We Need HPP (HTTP Parameter Pollution Protection)
// Example request: /api/v1/tours?sort=duration&sort=price. Because sort appears twice, Express turns it into an array: sort: ['duration', 'price']
// Your code expects a string, so: this.queryString.sort.split(','). So this throws the error: "split is not a function" → because .split() works on
// strings, not arrays.
// Attackers can intentionally send duplicate parameters to break your API.

// 2) Using HPP Middleware: HPP removes duplicate query parameters, Except for the ones listed in whitelist
app.use(
  hpp({
    whitelist: [
      'duration',
...
// 3) Why a Whitelist? Some parameters should be allowed to appear multiple times.
// Example: /api/v1/tours?duration=5&duration=9. This means:Find tours whose duration is 5 OR 9. If HPP removes duplicates, this stops working.
// So we tell HPP: “These fields (duration, difficulty, etc.) are allowed to be duplicated.” using the whitelist above

// 4) Where to Place It:
// Place HPP near the end of all middleware, Because it “cleans up” the query string.
// Ends here
