...
///THIS IS FOR THIS LESSON: Fc checks if the given password is the same as that stored in the doc in tthe database
// So for the first time, we  are gonna create smth called an Instance mthd. So an instances mthd is basically a mthd
// that is gonna available on all docs of a certain collectn and it works as seen in the below:
userSchema.methods.correctPassword = async function (
  canditatePassword, //This is password being passed
  userPassword, // This is password in the doc in the database
) {
  // Now inside of these instance mthds, since they are available on the doc, the "this" keyword pts to the current
  // doc. But in this case, since we ave the password set to "select: false", "this.password" will not be available.
  // And this is the reason we actually had to use the "userPassword,", except for this, we would use "this.password".
