...
// THIS IS FOR THIS LECTURE: Updating the Current User Password i.e Already logged in User
...
exports.updatePassword = catchAsync(async (req, res, next) => {
  // Now remember that this password update fclty is for logged in users, but we still we need the user to pass in his current password, so in order to
  // confirm that user actually is who he says he is. So jsut as a security measure bcos imagine that cmo would find ur computer open and then be able
  // to change passwors on the sites that u have currently open without being prompted for a password again. And so that would basically log u out of
  // all ur existing applications, which would of course, be a terrible experience. And so as a security measure, we always need to ask for the current
  // password b4 updating it, at all.
  ...
  // Now we did not use User.findByIdAndUpdate() bcos:
  //a) The validator we have in passwordConfirm in "userModel" is not going to work, bcos "this.password" is not defined when we use findByIDAndUpdate(),
  // bcos behind the scenes, Mongoose does not really keep the current object in memory, and it will not work
  //b) The pre-save middlewares we have in the "userModels" are also not going to work. So if we use "findByIdAndUpdate()", the password will not be encrpted
  // which is what the 1st middleware does and then also, the passwordChangedAt timestamp would also not be set.
  ...
  // The response we want to send here has been done in difft places in our code, solet's quickly create a fc for that
  // so we do not keep repeating the codes. So we will do that at the top part of this file
