...
// THIS IS FOR THIS LECTURE : handleDuplicateFieldsDB()
const handleValidationErrorDB = (err) => {
  // Now in order to create one big string out of all the errors, we basically have to loop over all of the objects in our response
...
// Now we could have made the above error handling alot more complete still, for e.g, we could define difft error severity levels like
// saying, this error is not so important, this error is medium important, and this error is very important or even critcal. And we
// could also then email sm adminitrator about critical errors etc. But again in a kind of small applicatn like this one, what we have
// here is already really good. So this is kind of a Robust strategy that we have implemented here.
// Now if we ever find another error we think is Operatnal, all we need to do is to do smth similar to the above we here so far.
// Ends here
...
module.exports = (err, req, res, next) => {
  ...
  if (process.env.NODE_ENV === 'development') {
    ...
    //Finally, let's now handle Mongoose's Validatn errors.
    // We can see this when we try to update a tour with sm invalid data and we got an error. So we used the below for updating:
    /* 
    { 
    "ratingsAverage": 6,
    "difficulty": "Whatever",
    ...
    */
    //The above object actually falls short of everything required in our tourSchema and so we will get sm errors in our response
    // We can see these errors when we are in "devpt". So we can switch to "npm start". U will the number of errors gotten bcos
    // all the fields in the object violates what was required. So i want to show u how we can create a meaningful error message
    // based on all the error response.
    // So inside our error,we get an error ppts and this ppt itself is an object which has a lot of objects in there, and each of
    // them is for one of the fields that has an error message and the messages are actually very meaningful. So what we want to do
    // is to extract each of the messages and put them all into one string
