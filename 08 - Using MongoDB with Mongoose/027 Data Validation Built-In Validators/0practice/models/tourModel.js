...
// Mongoose provides powerful tools for data validation, ensuring that values entered into a schema are in the correct
// format and that required fields are filled. Validation checks the correctness of data, while sanitization cleans the
// input to prevent malicious code (covered later in security).

// Mongoose includes built-in validators like required, min, max, and enum. We added themin the schema below
// The required option is a true validator that ensures a value is provided, while unique is not a validator — it just
// enforces uniqueness at the database level.

// In short, validation keeps your data accurate, and sanitization keeps your app safe.
...
// MONGOOSE MIDDLEWARE
// A) DOC MIDDLEWARE: These are middlewares that act directly on individual documents — i.e., instances of a Mongoose model. They run before
...
/*  ADD THIS TO the schema above so that we can create a secretTour: true
// 👇 Add this field
  secretTour: {
    type: Boolean,
    default: false,
...
//C) AGGREGATION MIDDLEWARE: To test this we use the Get tour Stats in postman
// Aggregation middleware -- in Mongoose lets you run code before or after an "aggregation pipeline" executes.
// You use it when you want to automatically modify or filter the aggregation pipeline — for example, to exclude secret
// documents from all aggregations without repeating code in every controller.
// aggregation pipeline -- is a sequence (or series) of stages that process data step by step — kind of like an assembly
// line in a factory. E.g 1) Finding The average price, OR The number of tours per difficulty level. Instead of doing many
// separate queries, you can use an aggregation pipeline that performs all these operations in one flow.
